...
def _conv_block(inputs, filters, alpha, kernel=(3, 3), strides=(1, 1), train_bn=True):
    ...
    #KL.BatchNormalization(axis=channel_axis, name='conv1_bn')(x)
...
def _depthwise_conv_block(inputs, pointwise_conv_filters, alpha,
    ...
    #KL.BatchNormalization(axis=channel_axis, name='conv_dw_%d_bn' % block_id)(x)
    ...
    #KL.BatchNormalization(axis=channel_axis, name='conv_pw_%d_bn' % block_id)(x)
...
def mobilenet_graph(img_input, architecture, alpha=1.0, depth_multiplier=1, train_bn=True):
    assert architecture in ["mobilenet"]
    # Stage 1
...
class PyramidROIAlign(KE.Layer):
    ...
    def call(self, inputs):
        ...
        for i, level in enumerate(range(2, 6)):
            ix = tf.to_int32(tf.where(tf.equal(roi_level, level)))
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    keep = tf.to_int32(tf.where(class_ids > 0))[:, 0]
    # Filter out low confidence boxes
    if config.DETECTION_MIN_CONFIDENCE:
        conf_keep = tf.to_int32(tf.where(class_scores >= config.DETECTION_MIN_CONFIDENCE))[:, 0]
        a = tf.expand_dims(keep, 0) 
        b = tf.expand_dims(conf_keep, 0)
        print("types", a.dtype, b.dtype)
        keep = tf.sets.set_intersection(a,b)
    ...
    def nms_keep_map(class_id):
        ...
        ixs = tf.to_int32(tf.where(tf.equal(pre_nms_class_ids, class_id)))[:, 0]
        # Apply NMS
    ...
    nms_keep = tf.reshape(tf.to_int32(nms_keep), [-1])
    nms_keep = tf.gather(nms_keep, tf.to_int32(tf.where(nms_keep > -1))[:, 0])
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        elif config.BACKBONE in ["mobilenet"]:
...
def mold_image(images, config):
    mold_image_dict = { 
            ...
            "mobilenet": mobilenet_mold_image
    }
...
def unmold_image(images, config):
    unmold_image_dict = {
            ...
            "mobilenet": mobilenet_unmold_image
    }
